...
const timestamps = {
    ...
    updatedAt: int({ mode: 'timestamp' }).notNull().default(sql`(CURRENT_TIMESTAMP)`),
}
...
export const providers = sqliteTable("providers", {
    ...
    name: text().notNull(),
    type: text('type', { enum: ['openai', 'anthropic', 'google'] }).notNull(), apiKey: text('api_key').notNull(),
...
export const chatMessages = sqliteTable('chat_messages', {
    ...
    message: text('message', { mode: 'json' }).$type<
        {
